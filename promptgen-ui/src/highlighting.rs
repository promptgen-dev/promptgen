...
impl TokenKind {
    /// Get the color for this token kind
    pub fn color(self) -> Color32 {
        match self {
            TokenKind::Text => syntax::TEXT,
            TokenKind::Reference => syntax::REFERENCE,
            TokenKind::Slot => syntax::SLOT,
            TokenKind::Option => syntax::OPTION,
            TokenKind::Brace => syntax::BRACE,
            TokenKind::Comment => syntax::COMMENT,
            TokenKind::Error => syntax::ERROR,
        }
    }
}
...
pub fn highlight_template(
    ...
    {
        highlight_from_ast(&mut job, text, ast, &font_id);
        return job;
    }

    // Fallback: simple regex-like highlighting for when parsing fails
    highlight_fallback(&mut job, text, &font_id);
    job
}
...
fn highlight_from_ast(
    ...
    for (node, span) in &ast.nodes {
        ...
        if span.start > text_len || span.end > text_len || span.start > span.end {
            ...
            if last_end < text_len {
                highlight_fallback_range(job, &text[last_end..], font_id);
            }
            return;
        }
        ...
        if span.start > last_end && last_end < text_len {
            ...
            append_token(job, &text[last_end..gap_end], TokenKind::Text, font_id);
        }
        ...
        match node {
            Node::Text(_) => {
                append_token(job, node_text, TokenKind::Text, font_id);
            }
            Node::LibraryRef(_) => {
                // Highlight @ symbol and the reference name
                append_token(job, node_text, TokenKind::Reference, font_id);
            }
            Node::Slot(_) => {
                // Highlight entire slot including {{ }}
                append_token(job, node_text, TokenKind::Slot, font_id);
            }
            Node::InlineOptions(_) => {
                // Highlight inline options with brace coloring for { and }
                highlight_inline_options(job, node_text, font_id);
            }
            Node::Comment(_) => {
                append_token(job, node_text, TokenKind::Comment, font_id);
            }
        }
    ...
    if last_end < text_len {
        append_token(job, &text[last_end..], TokenKind::Text, font_id);
    }
}

/// Fallback highlighting for a range when AST is stale
fn highlight_fallback_range(job: &mut LayoutJob, text: &str, font_id: &FontId) {
    highlight_fallback(job, text, font_id);
}

/// Highlight inline options with colored braces and pipe separators
fn highlight_inline_options(job: &mut LayoutJob, text: &str, font_id: &FontId) {
    // Text format: {option1|option2|option3}
    if text.starts_with('{') && text.ends_with('}') {
        // Opening brace
        append_token(job, "{", TokenKind::Brace, font_id);
        ...
        for (i, part) in parts.iter().enumerate() {
            append_token(job, part, TokenKind::Option, font_id);
            if i < parts.len() - 1 {
                append_token(job, "|", TokenKind::Brace, font_id);
            }
        }
        ...
        append_token(job, text, TokenKind::Option, font_id);
    }
}

/// Fallback highlighting when parsing fails - uses simple pattern matching
fn highlight_fallback(job: &mut LayoutJob, text: &str, font_id: &FontId) {
        ...
        match c {
            '@' => {
                ...
                if !current_text.is_empty() {
                    append_token(job, &current_text, TokenKind::Text, font_id);
                    current_text.clear();
                }
                ...
                if i + 1 < chars.len() && chars[i + 1] == '"' {
                    ...
                    append_token(job, &ref_text, TokenKind::Reference, font_id);
                } else {
            ...
            '{' => {
                ...
                if i + 1 < chars.len() && chars[i + 1] == '{' {
                    ...
                    append_token(job, &slot_text, TokenKind::Slot, font_id);
                } else {
                    // Inline options { ... }
                    let start = i;
                    let mut depth = 1;
                    ...
                    highlight_inline_options(job, &opt_text, font_id);
                }
                continue;
            }
            '#' => {
                ...
                append_token(job, &comment_text, TokenKind::Comment, font_id);
                continue;
            }
            _ => {
                current_text.push(c);
...
fn append_token(job: &mut LayoutJob, text: &str, kind: TokenKind, font_id: &FontId) {
    if text.is_empty() {
        return;
    }
